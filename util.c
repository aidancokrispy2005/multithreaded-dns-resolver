 ...
 * the Multithreaded DNS Resolver Assgnmt.
 *
 *
 */
...
int dnslookup(const char* hostname, char* firstIPstr, int maxSize){
    ...
    if(addrError){
    // commented out error log to prevent cluttered shell during testing
    //
    ...
    if(result->ai_addr->sa_family == AF_INET){
        ...
        freeaddrinfo(headresult); // Fix for valgrind leak
        return UTIL_FAILURE;
        }
#ifdef UTIL_DEBUG
        fprintf(stdout, "%s\n", ipv4str);
    ...
    */
    ...
    // Below is the "stub" code to completely abstract the DNS lookup
    // from the equation; used to test real time performance of the 
    // rest of the code.
    /*
    // Copy a fake, valid IP address
    strncpy(firstIPstr, "127.0.0.1", maxSize);
    // Ensure null termination (safe for maxSize < 1)
    if (maxSize > 0) {
        firstIPstr[maxSize - 1] = '\0';
    }
